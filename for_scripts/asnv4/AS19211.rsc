:global AddressList
/ip firewall address-list
:do {add list=$AddressList comment=AS19211 address=119.59.208.0/22} on-error {}
:do {add list=$AddressList comment=AS19211 address=119.59.236.0/22} on-error {}
:do {add list=$AddressList comment=AS19211 address=124.42.176.0/22} on-error {}
:do {add list=$AddressList comment=AS19211 address=130.12.76.0/22} on-error {}
:do {add list=$AddressList comment=AS19211 address=131.143.32.0/22} on-error {}
:do {add list=$AddressList comment=AS19211 address=131.143.8.0/22} on-error {}
:do {add list=$AddressList comment=AS19211 address=185.181.56.0/22} on-error {}
:do {add list=$AddressList comment=AS19211 address=195.172.152.0/21} on-error {}
:do {add list=$AddressList comment=AS19211 address=207.170.172.0/22} on-error {}
:do {add list=$AddressList comment=AS19211 address=213.169.220.0/22} on-error {}
:do {add list=$AddressList comment=AS19211 address=216.132.232.0/22} on-error {}
:do {add list=$AddressList comment=AS19211 address=216.235.216.0/22} on-error {}
:do {add list=$AddressList comment=AS19211 address=217.22.232.0/21} on-error {}
:do {add list=$AddressList comment=AS19211 address=45.83.112.0/22} on-error {}
:do {add list=$AddressList comment=AS19211 address=66.235.96.0/22} on-error {}
:do {add list=$AddressList comment=AS19211 address=72.245.168.0/21} on-error {}
:do {add list=$AddressList comment=AS19211 address=72.245.188.0/22} on-error {}
