:global AddressList
/ip firewall address-list
:if ([:len [find where list=$AddressList and address=187.40.80.0/23]] = 0) do={ add list=$AddressList comment=AS214155 address=187.40.80.0/23 }
:if ([:len [find where list=$AddressList and address=187.40.82.0/24]] = 0) do={ add list=$AddressList comment=AS214155 address=187.40.82.0/24 }
:if ([:len [find where list=$AddressList and address=187.40.84.0/24]] = 0) do={ add list=$AddressList comment=AS214155 address=187.40.84.0/24 }
:if ([:len [find where list=$AddressList and address=187.40.86.0/23]] = 0) do={ add list=$AddressList comment=AS214155 address=187.40.86.0/23 }
:if ([:len [find where list=$AddressList and address=187.40.88.0/23]] = 0) do={ add list=$AddressList comment=AS214155 address=187.40.88.0/23 }
:if ([:len [find where list=$AddressList and address=217.216.70.0/24]] = 0) do={ add list=$AddressList comment=AS214155 address=217.216.70.0/24 }
:if ([:len [find where list=$AddressList and address=91.218.0.0/22]] = 0) do={ add list=$AddressList comment=AS214155 address=91.218.0.0/22 }
:if ([:len [find where list=$AddressList and address=94.156.228.0/24]] = 0) do={ add list=$AddressList comment=AS214155 address=94.156.228.0/24 }
:if ([:len [find where list=$AddressList and address=94.156.31.0/24]] = 0) do={ add list=$AddressList comment=AS214155 address=94.156.31.0/24 }
