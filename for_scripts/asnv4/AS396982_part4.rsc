:global AddressList
/ip firewall address-list
:if ([:len [find where list=$AddressList and address=52.119.11.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=52.119.11.0/24 }
:if ([:len [find where list=$AddressList and address=57.140.170.0/23]] = 0) do={ add list=$AddressList comment=AS396982 address=57.140.170.0/23 }
:if ([:len [find where list=$AddressList and address=57.140.172.0/22]] = 0) do={ add list=$AddressList comment=AS396982 address=57.140.172.0/22 }
:if ([:len [find where list=$AddressList and address=57.140.184.0/22]] = 0) do={ add list=$AddressList comment=AS396982 address=57.140.184.0/22 }
:if ([:len [find where list=$AddressList and address=57.140.188.0/23]] = 0) do={ add list=$AddressList comment=AS396982 address=57.140.188.0/23 }
:if ([:len [find where list=$AddressList and address=62.122.31.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=62.122.31.0/24 }
:if ([:len [find where list=$AddressList and address=63.141.158.0/23]] = 0) do={ add list=$AddressList comment=AS396982 address=63.141.158.0/23 }
:if ([:len [find where list=$AddressList and address=64.18.136.0/23]] = 0) do={ add list=$AddressList comment=AS396982 address=64.18.136.0/23 }
:if ([:len [find where list=$AddressList and address=64.18.138.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=64.18.138.0/24 }
:if ([:len [find where list=$AddressList and address=64.52.19.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=64.52.19.0/24 }
:if ([:len [find where list=$AddressList and address=64.6.17.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=64.6.17.0/24 }
:if ([:len [find where list=$AddressList and address=64.6.18.0/23]] = 0) do={ add list=$AddressList comment=AS396982 address=64.6.18.0/23 }
:if ([:len [find where list=$AddressList and address=64.69.29.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=64.69.29.0/24 }
:if ([:len [find where list=$AddressList and address=64.69.30.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=64.69.30.0/24 }
:if ([:len [find where list=$AddressList and address=66.103.16.0/20]] = 0) do={ add list=$AddressList comment=AS396982 address=66.103.16.0/20 }
:if ([:len [find where list=$AddressList and address=66.170.109.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=66.170.109.0/24 }
:if ([:len [find where list=$AddressList and address=66.43.2.0/23]] = 0) do={ add list=$AddressList comment=AS396982 address=66.43.2.0/23 }
:if ([:len [find where list=$AddressList and address=66.45.103.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=66.45.103.0/24 }
:if ([:len [find where list=$AddressList and address=67.199.248.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=67.199.248.0/24 }
:if ([:len [find where list=$AddressList and address=67.219.242.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=67.219.242.0/24 }
:if ([:len [find where list=$AddressList and address=67.219.244.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=67.219.244.0/24 }
:if ([:len [find where list=$AddressList and address=67.219.246.0/23]] = 0) do={ add list=$AddressList comment=AS396982 address=67.219.246.0/23 }
:if ([:len [find where list=$AddressList and address=67.219.250.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=67.219.250.0/24 }
:if ([:len [find where list=$AddressList and address=69.48.216.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=69.48.216.0/24 }
:if ([:len [find where list=$AddressList and address=69.48.232.0/22]] = 0) do={ add list=$AddressList comment=AS396982 address=69.48.232.0/22 }
:if ([:len [find where list=$AddressList and address=69.5.191.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=69.5.191.0/24 }
:if ([:len [find where list=$AddressList and address=69.87.102.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=69.87.102.0/24 }
:if ([:len [find where list=$AddressList and address=69.87.107.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=69.87.107.0/24 }
:if ([:len [find where list=$AddressList and address=69.87.115.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=69.87.115.0/24 }
:if ([:len [find where list=$AddressList and address=69.87.119.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=69.87.119.0/24 }
:if ([:len [find where list=$AddressList and address=69.87.54.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=69.87.54.0/24 }
:if ([:len [find where list=$AddressList and address=72.1.88.0/21]] = 0) do={ add list=$AddressList comment=AS396982 address=72.1.88.0/21 }
:if ([:len [find where list=$AddressList and address=72.22.92.0/23]] = 0) do={ add list=$AddressList comment=AS396982 address=72.22.92.0/23 }
:if ([:len [find where list=$AddressList and address=74.112.185.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=74.112.185.0/24 }
:if ([:len [find where list=$AddressList and address=74.112.186.0/23]] = 0) do={ add list=$AddressList comment=AS396982 address=74.112.186.0/23 }
:if ([:len [find where list=$AddressList and address=74.112.188.0/23]] = 0) do={ add list=$AddressList comment=AS396982 address=74.112.188.0/23 }
:if ([:len [find where list=$AddressList and address=74.112.190.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=74.112.190.0/24 }
:if ([:len [find where list=$AddressList and address=74.117.40.0/21]] = 0) do={ add list=$AddressList comment=AS396982 address=74.117.40.0/21 }
:if ([:len [find where list=$AddressList and address=74.120.14.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=74.120.14.0/24 }
:if ([:len [find where list=$AddressList and address=74.221.130.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=74.221.130.0/24 }
:if ([:len [find where list=$AddressList and address=74.51.254.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=74.51.254.0/24 }
:if ([:len [find where list=$AddressList and address=75.125.0.0/18]] = 0) do={ add list=$AddressList comment=AS396982 address=75.125.0.0/18 }
:if ([:len [find where list=$AddressList and address=75.125.192.0/19]] = 0) do={ add list=$AddressList comment=AS396982 address=75.125.192.0/19 }
:if ([:len [find where list=$AddressList and address=75.125.224.0/23]] = 0) do={ add list=$AddressList comment=AS396982 address=75.125.224.0/23 }
:if ([:len [find where list=$AddressList and address=75.125.226.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=75.125.226.0/24 }
:if ([:len [find where list=$AddressList and address=75.125.236.0/22]] = 0) do={ add list=$AddressList comment=AS396982 address=75.125.236.0/22 }
:if ([:len [find where list=$AddressList and address=75.125.240.0/20]] = 0) do={ add list=$AddressList comment=AS396982 address=75.125.240.0/20 }
:if ([:len [find where list=$AddressList and address=76.76.19.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=76.76.19.0/24 }
:if ([:len [find where list=$AddressList and address=78.31.8.0/21]] = 0) do={ add list=$AddressList comment=AS396982 address=78.31.8.0/21 }
:if ([:len [find where list=$AddressList and address=79.140.192.0/23]] = 0) do={ add list=$AddressList comment=AS396982 address=79.140.192.0/23 }
:if ([:len [find where list=$AddressList and address=79.170.100.0/23]] = 0) do={ add list=$AddressList comment=AS396982 address=79.170.100.0/23 }
:if ([:len [find where list=$AddressList and address=79.170.102.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=79.170.102.0/24 }
:if ([:len [find where list=$AddressList and address=8.228.0.0/14]] = 0) do={ add list=$AddressList comment=AS396982 address=8.228.0.0/14 }
:if ([:len [find where list=$AddressList and address=8.232.0.0/14]] = 0) do={ add list=$AddressList comment=AS396982 address=8.232.0.0/14 }
:if ([:len [find where list=$AddressList and address=8.236.128.0/21]] = 0) do={ add list=$AddressList comment=AS396982 address=8.236.128.0/21 }
:if ([:len [find where list=$AddressList and address=8.236.144.0/20]] = 0) do={ add list=$AddressList comment=AS396982 address=8.236.144.0/20 }
:if ([:len [find where list=$AddressList and address=8.236.192.0/18]] = 0) do={ add list=$AddressList comment=AS396982 address=8.236.192.0/18 }
:if ([:len [find where list=$AddressList and address=8.237.0.0/17]] = 0) do={ add list=$AddressList comment=AS396982 address=8.237.0.0/17 }
:if ([:len [find where list=$AddressList and address=8.34.208.0/20]] = 0) do={ add list=$AddressList comment=AS396982 address=8.34.208.0/20 }
:if ([:len [find where list=$AddressList and address=8.35.192.0/20]] = 0) do={ add list=$AddressList comment=AS396982 address=8.35.192.0/20 }
:if ([:len [find where list=$AddressList and address=80.254.192.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=80.254.192.0/24 }
:if ([:len [find where list=$AddressList and address=80.91.225.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=80.91.225.0/24 }
:if ([:len [find where list=$AddressList and address=85.120.176.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=85.120.176.0/24 }
:if ([:len [find where list=$AddressList and address=85.158.142.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=85.158.142.0/24 }
:if ([:len [find where list=$AddressList and address=85.208.98.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=85.208.98.0/24 }
:if ([:len [find where list=$AddressList and address=87.192.200.0/23]] = 0) do={ add list=$AddressList comment=AS396982 address=87.192.200.0/23 }
:if ([:len [find where list=$AddressList and address=87.238.241.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=87.238.241.0/24 }
:if ([:len [find where list=$AddressList and address=87.238.242.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=87.238.242.0/24 }
:if ([:len [find where list=$AddressList and address=87.238.244.0/23]] = 0) do={ add list=$AddressList comment=AS396982 address=87.238.244.0/23 }
:if ([:len [find where list=$AddressList and address=87.251.20.0/23]] = 0) do={ add list=$AddressList comment=AS396982 address=87.251.20.0/23 }
:if ([:len [find where list=$AddressList and address=91.197.141.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=91.197.141.0/24 }
:if ([:len [find where list=$AddressList and address=91.199.181.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=91.199.181.0/24 }
:if ([:len [find where list=$AddressList and address=91.213.126.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=91.213.126.0/24 }
:if ([:len [find where list=$AddressList and address=91.241.14.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=91.241.14.0/24 }
:if ([:len [find where list=$AddressList and address=92.119.244.0/23]] = 0) do={ add list=$AddressList comment=AS396982 address=92.119.244.0/23 }
:if ([:len [find where list=$AddressList and address=92.119.246.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=92.119.246.0/24 }
:if ([:len [find where list=$AddressList and address=94.156.176.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=94.156.176.0/24 }
:if ([:len [find where list=$AddressList and address=95.135.31.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=95.135.31.0/24 }
:if ([:len [find where list=$AddressList and address=98.158.240.0/20]] = 0) do={ add list=$AddressList comment=AS396982 address=98.158.240.0/20 }
:if ([:len [find where list=$AddressList and address=98.98.157.0/24]] = 0) do={ add list=$AddressList comment=AS396982 address=98.98.157.0/24 }
