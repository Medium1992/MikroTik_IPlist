:global AddressList
/ip firewall address-list
:do {add list=$AddressList comment=AS44620 address=158.94.216.0/24} on-error {}
:do {add list=$AddressList comment=AS44620 address=212.87.198.0/23} on-error {}
:do {add list=$AddressList comment=AS44620 address=217.11.164.0/24} on-error {}
:do {add list=$AddressList comment=AS44620 address=37.221.79.0/24} on-error {}
:do {add list=$AddressList comment=AS44620 address=45.74.158.0/23} on-error {}
:do {add list=$AddressList comment=AS44620 address=77.92.151.0/24} on-error {}
:do {add list=$AddressList comment=AS44620 address=91.228.227.0/24} on-error {}
:do {add list=$AddressList comment=AS44620 address=92.249.60.0/24} on-error {}
