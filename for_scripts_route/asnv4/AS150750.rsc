:global Distance
:global RouteTab
:global GateWay
/ip route
:if ([:len [/ip/route/find dst-address=103.145.185.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=103.145.185.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS150750 }
:if ([:len [/ip/route/find dst-address=103.162.136.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=103.162.136.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS150750 }
:if ([:len [/ip/route/find dst-address=103.83.30.0/23 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=103.83.30.0/23 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS150750 }
:if ([:len [/ip/route/find dst-address=113.192.47.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=113.192.47.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS150750 }
:if ([:len [/ip/route/find dst-address=116.90.119.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=116.90.119.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS150750 }
:if ([:len [/ip/route/find dst-address=144.79.57.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=144.79.57.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS150750 }
:if ([:len [/ip/route/find dst-address=151.123.144.0/23 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=151.123.144.0/23 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS150750 }
:if ([:len [/ip/route/find dst-address=154.208.32.0/19 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=154.208.32.0/19 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS150750 }
:if ([:len [/ip/route/find dst-address=162.4.162.0/23 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=162.4.162.0/23 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS150750 }
:if ([:len [/ip/route/find dst-address=162.4.192.0/23 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=162.4.192.0/23 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS150750 }
:if ([:len [/ip/route/find dst-address=59.103.45.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=59.103.45.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS150750 }
