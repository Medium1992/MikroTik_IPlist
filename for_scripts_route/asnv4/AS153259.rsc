:global Distance
:global RouteTab
:global GateWay
/ip route
:if ([:len [/ip/route/find dst-address=128.1.122.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=128.1.122.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS153259 }
:if ([:len [/ip/route/find dst-address=128.128.80.0/22 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=128.128.80.0/22 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS153259 }
:if ([:len [/ip/route/find dst-address=128.128.84.0/23 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=128.128.84.0/23 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS153259 }
:if ([:len [/ip/route/find dst-address=128.128.87.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=128.128.87.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS153259 }
:if ([:len [/ip/route/find dst-address=128.128.90.0/23 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=128.128.90.0/23 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS153259 }
:if ([:len [/ip/route/find dst-address=128.128.92.0/23 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=128.128.92.0/23 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS153259 }
:if ([:len [/ip/route/find dst-address=128.128.95.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=128.128.95.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS153259 }
:if ([:len [/ip/route/find dst-address=129.227.254.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=129.227.254.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS153259 }
:if ([:len [/ip/route/find dst-address=156.59.247.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=156.59.247.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS153259 }
:if ([:len [/ip/route/find dst-address=160.250.120.0/23 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=160.250.120.0/23 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS153259 }
:if ([:len [/ip/route/find dst-address=164.90.104.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=164.90.104.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS153259 }
:if ([:len [/ip/route/find dst-address=164.90.123.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=164.90.123.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS153259 }
:if ([:len [/ip/route/find dst-address=164.90.73.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=164.90.73.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS153259 }
:if ([:len [/ip/route/find dst-address=164.90.97.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=164.90.97.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS153259 }
:if ([:len [/ip/route/find dst-address=169.136.109.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=169.136.109.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS153259 }
:if ([:len [/ip/route/find dst-address=98.98.224.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=98.98.224.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS153259 }
