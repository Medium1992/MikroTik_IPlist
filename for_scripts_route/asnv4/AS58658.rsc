:global Distance
:global RouteTab
:global GateWay
/ip route
:if ([:len [/ip/route/find dst-address=149.29.112.0/21 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=149.29.112.0/21 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS58658 }
:if ([:len [/ip/route/find dst-address=149.29.120.0/22 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=149.29.120.0/22 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS58658 }
:if ([:len [/ip/route/find dst-address=149.29.64.0/19 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=149.29.64.0/19 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS58658 }
:if ([:len [/ip/route/find dst-address=149.29.96.0/20 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=149.29.96.0/20 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS58658 }
