:global Distance
:global RouteTab
:global GateWay
/ip route
:if ([:len [/ip/route/find dst-address=217.60.92.0/22 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=217.60.92.0/22 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS59447 }
:if ([:len [/ip/route/find dst-address=77.110.66.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=77.110.66.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS59447 }
:if ([:len [/ip/route/find dst-address=77.110.75.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=77.110.75.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS59447 }
:if ([:len [/ip/route/find dst-address=77.110.77.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=77.110.77.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS59447 }
