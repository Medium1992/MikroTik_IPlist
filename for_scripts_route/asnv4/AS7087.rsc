:global Distance
:global RouteTab
:global GateWay
/ip route
:if ([:len [/ip/route/find dst-address=200.13.193.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=200.13.193.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS7087 }
:if ([:len [/ip/route/find dst-address=200.13.194.0/23 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=200.13.194.0/23 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS7087 }
:if ([:len [/ip/route/find dst-address=200.13.196.0/22 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=200.13.196.0/22 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS7087 }
:if ([:len [/ip/route/find dst-address=200.13.200.0/21 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=200.13.200.0/21 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS7087 }
:if ([:len [/ip/route/find dst-address=200.13.208.0/22 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=200.13.208.0/22 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS7087 }
:if ([:len [/ip/route/find dst-address=200.13.212.0/23 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=200.13.212.0/23 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS7087 }
:if ([:len [/ip/route/find dst-address=200.13.214.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=200.13.214.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS7087 }
:if ([:len [/ip/route/find dst-address=200.13.216.0/21 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=200.13.216.0/21 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=AS7087 }
