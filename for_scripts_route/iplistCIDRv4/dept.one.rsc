...
:global GateWay
/ip route
:if ([:len [/ip/route/find comment=dept.one and dst-address=for_scripts_route/iplistCIDRv4/dept.one.rsc]] = 0) do={ add dst-address=for_scripts_route/iplistCIDRv4/dept.one.rsc gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=dept.one }
